...
export async function fetchWeatherForecast(
  ...
  // Use browser's IANA timezone so daily boundaries align with the calendar
  const tz = Intl.DateTimeFormat().resolvedOptions().timeZone
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&daily=temperature_2m_max,temperature_2m_min,apparent_temperature_max,apparent_temperature_min,weather_code&hourly=apparent_temperature&temperature_unit=fahrenheit&timezone=${encodeURIComponent(tz)}&forecast_days=16`
