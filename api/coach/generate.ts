...
const SYSTEM_PROMPT = `You are an expert marathon coach creating a personalized training plan. You understand periodization, progressive overload, recovery, tapering, and cross-training.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Build week start dates for the prompt
  const weekStarts = Array.from({ length: planWeeks }, (_, i) =>
    format(addDays(planStart, i * 7), 'yyyy-MM-dd')
  )

  const goalText = raceGoal === 'time_target' && goalTimeSeconds
    ? `Target time: ${formatTime(goalTimeSeconds)}`
    : raceGoal === 'bq' ? 'Boston Qualifying time'
    : raceGoal === 'pr' ? 'Personal Record'
    : 'Finish the race'

  const distanceLabel = {
    marathon: 'marathon (26.2 miles)',
    half_marathon: 'half marathon (13.1 miles)',
    '10k': '10K (6.2 miles)',
    '5k': '5K (3.1 miles)',
...
Each week should have exactly 7 days (Monday through Sunday). Use the week start dates provided. Generate the complete plan as JSON.`
  ...
  try {
    ...
    // Save to Supabase — archive any existing active plan first
    await supabase
      .from('coach_plans')
    ...
    if (insertError) {
      ...
      // Still return the plan even if save fails — client can retry save
    }
